...
public class SwipeActionHelper extends RecyclerView.ItemDecoration {
    ...
    public RecyclerView.ViewHolder getActive() {
        return swipeActionDelegate.getActive(recyclerView);
    }
    ...
    void notifyExpand(RecyclerView.ViewHolder viewHolder) {
        if (onSwipeActionListener != null) {
            onSwipeActionListener.onExpandChanged(this, viewHolder);
        }
    }

    void notifySwipeChanged(boolean whenDraw) {
        ...
        if (newValue ^ isSwiped) {
            ...
            if (onSwipeActionListener != null) {
                recyclerView.post(() -> onSwipeActionListener.onSwipeChanged(this, oldValue, newValue));
            }
        }
...
//            super.onChildDrawOver(c, recyclerView, viewHolder, dX, dY, actionState, isCurrentlyActive);
                    ...
                    if (activeHolder != null) {
                        if (view == null) {
                            ...
                            if (activeHolder != underHolder) {
                                ...
                                List<View> list = swipeActionDelegate.getTouchable(underHolder);
                                for (View v: list) {
                                    rect.set(0, 0, v.getWidth(), v.getHeight());
                                    rv.offsetDescendantRectToMyCoords(v, rect);
                                    if (hitTest(v, e.getX(), e.getY(), rect.left, rect.top)) {
                                        result = false;
                                        break;
                                    }
                                }
    ...
    private static class SwipeActionDelegate {
        ...
        public void clear(RecyclerView recyclerView) {
            ...
            if (helper.onSwipeActionListener != null) {
                helper.onSwipeActionListener.onExpandChanged(helper, null);
            }
        }
        ...
        public boolean isSwiped(RecyclerView recyclerView) {
            ...
            for (int i = 0, count = recyclerView.getChildCount(); i < count; i++) {
                ...
                RecyclerView.ViewHolder holder = recyclerView.findContainingViewHolder(view);
                Adapter adapter = this.getAdapter(holder);
                if (adapter != null && holder.getAdapterPosition() >= 0 && adapter.isSwiped(helper)) {
                    return true;
                }
            }
        ...
        public List<View> getTouchable(RecyclerView.ViewHolder viewHolder) {
            Adapter adapter = this.getAdapter(viewHolder);
            if (adapter != null) {
                return adapter.getTouchable(helper);
            }
    ...
    public interface OnSwipeActionListener {
        ...
        default void onExpandChanged(SwipeActionHelper helper, RecyclerView.ViewHolder viewHolder) {}
    }
