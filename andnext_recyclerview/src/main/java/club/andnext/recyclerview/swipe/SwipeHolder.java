...
public class SwipeHolder {
    ...
    public boolean expand(final int direction, final boolean smooth) {
        if (activeRunner != null) {
            return false;
        }

        Optional<SwipeRunner> runner = list.stream()
                .filter(r -> (r.getDirection() == direction))
                .findAny();
    ...
    public void notifyExpand(SwipeRunner runner) {
        if (runner != this.activeRunner) {
            return;
        }
        ...
        if (viewHolder != null) {
            helper.notifyExpand(viewHolder);
        }
    }
}
